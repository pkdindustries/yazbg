...
fn createNewEntity() void {
    ...
    // Random size
    const size = rng.float(f32) * 20 + 0.5;
    ...
    // Randomly apply static shader to 1% of entities
    if (rng.intRangeAtMost(u8, 0, 99) == 0) {
        const current_time = @as(f32, @floatCast(ray.GetTime()));

        // Make the entity larger for better visibility of the shader effect
        ...
        shaders.addShaderToEntity(entity, "static") catch |err| {
            std.debug.print("Failed to add static shader to entity: {}\n", .{err});
        };

        // Set initial time uniform for the shader
        // Note: We need a mutable reference to the shader component to modify uniforms
        if (ecs.has(components.Shader, entity)) {
            var shader_component = ecs.getUnchecked(components.Shader, entity);
            shader_component.setFloat("time", current_time) catch |err| {
                std.debug.print("Error setting time uniform: {}\n", .{err});
            };
        }
    }
...
fn runFrame(timer: *std.time.Timer, frame_count: *u32, total_anim_time: *u64, total_render_time: *u64) void {
    ...
    const to_create = if (target_entities <= current_entity_count)
        0 // Avoid subtraction if we've already met or exceeded the target
    else
        @min(target_entities - current_entity_count, max_new_per_frame);
