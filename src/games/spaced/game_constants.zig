// game_constants.zig - Spaced game constants
pub const PLAYER_SPEED: f32 = 200.0; // pixels per second
pub const PLAYER_SIZE: f32 = 16.0; // player sprite size
pub const ENEMY_SPEED: f32 = 50.0; // pixels per second
pub const ENEMY_SIZE: f32 = 12.0; // enemy sprite size

pub const PLAYER_MAX_HEALTH: f32 = 100.0;
pub const PLAYER_START_HEALTH: f32 = 100.0;

// World bounds - match window logical resolution for now
pub const WORLD_WIDTH: f32 = 640.0;
pub const WORLD_HEIGHT: f32 = 760.0;

// Camera
pub const CAMERA_ZOOM: f32 = 1.0;